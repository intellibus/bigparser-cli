# bigparser-cli design

## Table of Contents

- [bigparser-cli design](#bigparser-cli-design)
...
`bigparser-cli` is designed to locally persist authentication credentials for `bigparser` APIs, similar to the `aws` cli.

## How is it different from what I already use?

**WORK IN PROGRESS**: `bigparser-cli` is still in the `design` phase.
...
`bigparser-cli` is a part of the [Open Source Universe](https://github.com/intellibus/approach). All Projects in the [Open Source Universe](https://github.com/intellibus/approach) follow the same [Contribution Guidelines & Process](https://github.com/intellibus/approach).
...
`bigparser-cli` only plans to initially support Typescript & JavaScript Applications. However, we would love for the community to contribute packages compatible with other languages (Java, Go, etc.)
